...
//				heights[x + z * SIZE] = world.getChunkGenerator().getHeight((cx + x) * Chunk.SIZE, (cz + z) * Chunk.SIZE);
				heights[x + z * SIZE] = 5;
			}
		}
	}
	...
	public void update() {
		...
		for (int x = 0; x < size; x++) {
			for (int z = 0; z < size; z++) {
				...
				int X = x - size / 2;
				int Z = z - size / 2;
				int nc = PixelInventor.game.raytracer.getWorld().NUM_CHUNKS;
				if (X >= -nc / 2 && Z >= -nc / 2 && X <= nc / 2 && Z <= nc / 2) continue;
