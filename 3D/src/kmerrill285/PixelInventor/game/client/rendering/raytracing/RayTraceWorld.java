...
public class RayTraceWorld {
	...
	public RayTraceWorld() {
		...
		if (!built) {
			...
			GL43.glBindBuffer(GL43.GL_SHADER_STORAGE_BUFFER, FBUFFER);
			GL45.glNamedBufferStorage(FBUFFER, PixelInventor.game.world.heightmap.heights, GL45.GL_DYNAMIC_STORAGE_BIT);
			GL45.glBindBufferBase(GL45.GL_SHADER_STORAGE_BUFFER, 6, FBUFFER);
			GL43.glBindBuffer(GL43.GL_SHADER_STORAGE_BUFFER, 0);
...
//		this.clearVoxels();
	...
	public void build(RayShader shader) {
	    ...
	    GL45.glBindBuffer(GL45.GL_SHADER_STORAGE_BUFFER, FBUFFER);
	    GL45.glBufferSubData(GL45.GL_SHADER_STORAGE_BUFFER, 0, PixelInventor.game.world.heightmap.heights);
	    GL45.glBindBuffer(GL45.GL_SHADER_STORAGE_BUFFER, 0);
		...
		shader.setInt("far_plane_distance", PixelInventor.game.world.heightmap.SIZE);
	...
	public void dispose() {
		if (built == false) {
			...
			GL43.glDeleteBuffers(TBUFFER);
			GL43.glDeleteBuffers(FBUFFER);
			built = false;
		}
		RUNNING = false;
	}
