...
public class L {
	   ...
	   private HashMap<String, Color> stuff = new HashMap<String, Color>();
	   
	   public double angle = 30;
	   
	   public L(LSystemPos pos, LSystem parent, String[] system, int iterations, double size, boolean main, HashMap<String, Color> stuff) {
		   this.pos = pos;
		   this.system = system;
...
//		   splitTrueRand(rand);
	  ...
	  double sizeDown = 0.8;
	   public void splitTrueRand(Random rand) {
		   L l = new L(new LSystemPos(pos.getX() + 0,pos.getY() + 0, pos.getZ() + 0), parent, system, iterations+1, size*sizeDown,false,stuff);
...
//					   placeLeaf(x + rand.nextInt(3) - 1, y + rand.nextInt(3) - 1, z + rand.nextInt(3) - 1);
	   ...
	   public void placeLeaf(int x, int y, int z) {
		   if (stuff.get(x+","+y+","+z) == null)
			   stuff.put(x+","+y+","+z, Color.green);
	   }
