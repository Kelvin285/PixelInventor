...
public class MountainsBiome extends Biome {
	...
	public MountainsBiome() {
		super(Tiles.GRASS, Tiles.DIRT, Tiles.SMOOTH_STONE, BiomeHumidity.MODERATE, BiomeTemperature.COOL, BiomeLocation.LAND, BiomeType.MOUNTAIN);
	}
	...
	public float getHeightAt(float x, float y, float z, FastNoise noise) {
		return Math.abs(MathHelper.average(noise.GetPerlinFractal(x * 0.25f, z * 0.25f), noise.GetPerlinFractal(x, z))) * 80 + 
				Math.abs(noise.GetPerlinFractal(x * 0.35f, z * 0.35f)) * 10 * 50;
	}
