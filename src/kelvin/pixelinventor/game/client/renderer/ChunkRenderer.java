...
public class ChunkRenderer {
	public static void render(Tile[][] tiles, int[][][] states, double[][][] lights, int[] skyColor, Chunk chunk, BufferedImage image, BufferedImage lightImage) {
		...
		g = lightImage.getGraphics();
//		g.setColor(new Color(skyColor[0], skyColor[1], skyColor[2]));
//		g.fillRect(0, 0, image.getWidth(), image.getHeight());
		g.clearRect(0, 0, image.getWidth(), image.getHeight());
		g.dispose();
		
		double[][][] pixels = new double[lightImage.getWidth()][lightImage.getHeight()][4];
		
		for (int xx = 0; xx < lightImage.getWidth(); xx++) {
			for (int yy = 0; yy < lightImage.getHeight(); yy++) {
				pixels[xx][yy] = lightImage.getRaster().getPixel(xx, yy, pixels[xx][yy]);
				...
				if (Settings.smoothLights) {
					...
					if (Math.abs(R - skyColor[0]) < th && Math.abs(G - skyColor[1]) < th && Math.abs(B - skyColor[2]) < th) {
						...
						pixel[1] = G;
						pixel[2] = B;
						lightImage.getRaster().setPixel(xx, yy, pixel);
						continue;
					}
					...
					for (int xx1 = -2; xx1 < 3; xx1++) {
						for (int yy1 = -2; yy1 < 3; yy1++) {
							double[] l2 = chunk.getRenderLight(BX + xx1, BY + yy1);
							//(1.0 - dist)
							double dist = MathFunc.distance(RBX * size, RBY * size, (BX + xx1) * size + size, (BY + yy1) * size + size);
							dist /= size;
							if (dist <= 1.5) {
								dist /= 1.5;
