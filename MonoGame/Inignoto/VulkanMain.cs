...
using ImageLayout = SharpVk.ImageLayout;
...
namespace Inignoto
{
    public class VulkanMain
    {
        ...
        private void InitialiseWindow()
        {
            ...
            this.window.KeyDown += (x, args) => {
                foreach (InputSetting setting in InputSetting.inputs)
                {
                    if (!setting.Mouse)
                    {
                        if ((int)args.KeyCode == setting.Input)
                        {
                            setting.VK_PRESSED = true;
                        }
                    }
                }
            };

            this.window.KeyUp += (x, args) => {
                foreach (InputSetting setting in InputSetting.inputs)
                {
                    if (!setting.Mouse)
                    {
                        if ((int)args.KeyCode == setting.Input)
                        {
                            setting.VK_PRESSED = false;
                        }
                    }
                }
            };

            this.window.MouseDown += (x, args) => {
                foreach (InputSetting setting in InputSetting.inputs)
                {
                    if (setting.Mouse)
                    {
                        if ((int)args.Button == setting.Input)
                        {
                            setting.VK_PRESSED = true;
                        }
                    }
        ...
        private void InitialiseVulkan()
        {
            ...
            if (Inignoto.game.VULKAN_ENABLED == false) return;
        ...
        public Extent2D ChooseSwapExtent(SurfaceCapabilities capabilities)
        {
            ...
            else
            {
                return new Extent2D
                {
                    Width = System.Math.Max(capabilities.MinImageExtent.Width, System.Math.Min(capabilities.MaxImageExtent.Width, SurfaceWidth)),
                    Height = System.Math.Max(capabilities.MinImageExtent.Height, System.Math.Min(capabilities.MaxImageExtent.Height, SurfaceHeight))
                };
            }
        }
