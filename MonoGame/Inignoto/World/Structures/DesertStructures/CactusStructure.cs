...
namespace Inignoto.World.Structures.DesertStructures
{
    public class CactusStructure : Structure
    {
        ...
        {
            if (chunk.GetVoxel(chunk_x, chunk_y - 1, chunk_z) == TileRegistry.SAND.DefaultData)
            {
                    ...
                    if (n <= 2 && n > 0)
                    {
                        ...
                        for (int i = 0; i < height; i++)
                        {
                            if (chunk.GetVoxel(chunk_x, chunk_y + i, chunk_z) == TileRegistry.AIR.DefaultData)
                            {
                                if (i == height - 1)
                                {
                                    chunk.SetVoxel(chunk_x, chunk_y + i, chunk_z, TileRegistry.CACTUS.stateHolder.data[5]);
                                } else
                                {
                                    chunk.SetVoxel(chunk_x, chunk_y + i, chunk_z, TileRegistry.CACTUS.DefaultData);
                                }
                            }
                        }
                    }
